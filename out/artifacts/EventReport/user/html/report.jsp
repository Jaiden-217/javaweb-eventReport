...
<form id="submit_form" name="submit_form" method="post" action="/EventReport/SubmitReport" onsubmit="return submitClicked()">
    ...
    <div id="con">
        <div class="happen">
            <i class="note">*</i>
            <span>事件类型</span>
            <select id="category" name="category" oninput="onInputEvent(this)">
        ...
        <div class="happen">
            <i class="note">*</i>
            <span>发生日期</span>
            <input type="date" id="date" name="date" oninput="onInputEvent(this)" >
        </div>
        <div class="happen">
            <i class="note">*</i>
            <span>发生时间</span>
            <input type="time" id="time" name="time" oninput="onInputEvent(this)">
        </div>
        <div class="happen">
            <i class="note">*</i>
            <span>发生科室</span>
            <select id="department" name="department" oninput="onDepartmentChanged()">
        ...
        <div class="mode">
            <i class="note">*</i>
            <span>事件经过</span>
            <textarea cols="30" rows="10" id="course" name="course" oninput="onInputEvent(this)"></textarea>
        </div>
        <div class="mode">
            <i class="note">*</i>
            <span>事件结果与补救措施</span>
            <textarea cols="30" rows="10" id="result" name="result" oninput="onInputEvent(this)"></textarea>
        </div>
        <div class="mode">
            <i class="note">*</i>
            ...
            <div class="divAboveSelect">
                <select id="nurse_reason" name="nurse_reason" oninput="onSelected(this)" multiple="multiple" size="4">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="nurse_reason_ex" name="nurse_reason" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>病人因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="patient_reason" name="patient_reason" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="patient_reason_ex" name="patient_reason" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>环境因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="environ_reason" name="environ_reason" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="environ_reason_ex" name="environ_reason" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>管理因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="manage_reason" name="manage_reason" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="manage_reason_ex" name="manage_reason" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>
        </div>
        <div class="mode">
            <i class="note">*</i>
            ...
            <br>
            <div class="divAboveSelect">
                <select id="pro_nurse" name="pro_nurse" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="pro_nurse_ex" name="pro_nurse" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>病人因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="pro_patient" name="pro_patient" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="pro_patient_ex" name="pro_patient" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>环境因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="pro_environ" name="pro_environ" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="pro_environ_ex" name="pro_environ" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>

            <h4>管理因素:</h4>
            <br>
            <div class="divAboveSelect">
                <select id="pro_manage" name="pro_manage" oninput="onSelected(this)" multiple="multiple">
            ...
            <br>
            <div class="divUnderSelect">其他:<input type="text" id="pro_manage_ex" name="pro_manage" oninput="onInputEvent(this)"></div>
            <br><hr class="hrDiv"><br>
        </div>
        <div class="name happen">
            <span>提交人姓名</span>
            <input id="user_name" name="user_name" readonly="readonly" value="<%=session.getAttribute("userName")%>" style="width: 40%; height: 25px; text-align: center">
        </div>
        <div class="number happen">
            <span>提交人工号</span>
            <input id="user_id" name="user_id" readonly="readonly" value="<%=session.getAttribute("userId")%>" style="width: 40%; height: 25px; text-align: center">
        </div>
        <div class="subBtn">
            <input type="button" class="updateOrBack" id="subBtn" onclick="checkIsDepRight()" value="提交">
        </div>
    </div>
</form>
...
<script>
    ...
    function checkIsDepRight() {
        if (!isAllFilled()) {
            alert("存在未填的选项！");
        } else {
            var depId = "<%=session.getAttribute("departmentId")%>";
            var newDepId = "<%=session.getAttribute("new_department_id")%>";
            if (depId!=newDepId) {
                alert("请正确选择您所在的科室！");
            } else {
                showConfirm();
    ...
    function submitClicked() {
        $("#boxFarther1").show();
        $("#submit_form").ajaxSubmit(function (msg) {
            if (msg=="提交成功")
                clearCookie();
            hideBox();
            alert(msg);
    ...
    function isAllFilled() {
        if (jsonObj.date==undefined || jsonObj.date.length<1)
            jsonObj.date=getDefaultDate();

        if (jsonObj.time==undefined || jsonObj.time.length<1)
            return false;

        if (document.getElementById("course").value.length<2 || document.getElementById("result").value.length<2)
            return false;
    ...
    function checkSelect() {
        var completeCount = 0;
        var names = ["nurse_reason","environ_reason","manage_reason","pro_nurse","pro_environ","pro_manage"];
        for (var i = 0; i < names.length; i++) {
            var selects = document.getElementsByName(names[i]);
    ...
    function onInputEvent(e) {
        if (e.id == "category" && e.value=="护理安全事件-其他") {
            var category = "护理安全事件-"+prompt("请输入事件类型");
            var option = "<option value='"+category+"'>"+category+"</option>"
            $("#group_first").append(option);
            $("#category").val(category);
        } else if (e.id == "category" && e.value=="其他事件") {
            var category = "其他事件-"+prompt("请输入事件类型");
            var option = "<option value='"+category+"'>"+category+"</option>"
        ...
        // 跟随输入更新缓存
        localStorage.cookie=JSON.stringify(jsonObj);
    }
    ...
    function onDepartmentChanged() {
        ...
        xmlhttp.onreadystatechange=function() {
            if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                location.reload();
                document.getElementById("depDisplay").innerText="<%=session.getAttribute("department")%>:<%=session.getAttribute("leader_name")%>";
            }
        }
        var dep = document.getElementById("department").value;
        xmlhttp.open("GET","/EventReport/SubmitReport?department="+dep,true);
        xmlhttp.send();
    }

    function readCookie() {
        if (localStorage.cookie!=undefined)
            jsonObj = JSON.parse(localStorage.cookie);
        displayCookie(document.getElementById("course"));
        displayCookie(document.getElementById("result"));

        var inputs = document.getElementsByTagName("input");
        for (var i=0; i<inputs.length; i++) {
            var input = inputs[i];
            if (jsonObj[input.id] == undefined)
                continue
            input.value=jsonObj[input.id];
        }

        var selects = document.getElementsByTagName("select");
        for (var i=0; i<selects.length; i++) {
            var select = selects[i];
            if (jsonObj[select.id] == undefined)
                continue;

            if (i < 2) {
                select.value = jsonObj[select.id];
                continue;
            }

            var selectedItems = jsonObj[select.id].split(",");

            for (var j = 0; j<selectedItems.length; j++) {
                var index = parseInt(selectedItems[j]);
                if (isNaN(index))
                    continue;
    ...
    function displayCookie(e) {
        if (e.id == "department" && jsonObj[e.id] == undefined) {
            e.value="<%=session.getAttribute("department")%>";
            return;
        }
        if (jsonObj[e.id] != undefined && jsonObj[e.id].length > 1)
            e.value=jsonObj[e.id];
    }
